              ...
              <div class="panel panel-default">
                ...
                <div id="rayosx_dental" class="panel-collapse collapse in">
                  <table>
                    <tbody><tr>
                      <td>
                        <label class="checkbox-inline"><input type="checkbox" class="chkExamenes" id="panoramica_maxilares" ng-checked="panoramica_maxilares" name="examenes[303]">Panorámica de Maxilares</label>
                      </td>
                      <td>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
          ...
          $('#tbl_pacientes').DataTable({
              ...
              "pacientes[]": "Debe escoger al menos un paciente.",
        ...
        function checked(){
          ...
          //todos los checked - textarea
           var all2 = $( "#"+divpadre2+" input:checked" ).length;
           //todos los checked - check
           var all = $( "#"+divpadre+" input:checked" ).length;
          //todos los checked - textarea
          ...
          console.log(all);
           ...
           $('#'+divpadre+' .textarea').each(function(){
          ...
          $('#'+divpadre2+' .textarea').each(function(){
            if($.trim($(this).val())){
              existe = 1;//si al menos uno tiene algun valor diferente a espacios en blanco, existe será igual a 1
            console.log($(this).attr("id"));
          }
         });
            console.log(divpadre+' '+divpadre+' '+existe+' '+all+' '+all2)
          // Validación para cambiar color de pestaña
           if(existe == 1 || all >=1 || (divpadre2 != 'accordion' && all2 >=1)){// algún texarea tiene valor, o existe algún checked.
            //console.log(all);
             ...
             // console.log(divpadre+ "id"+divpadre2);
             $("#message").html("");
           }
      ...
      app.controller("controllerLaboratorio", function ($scope, $http, API_URL)
      {
        ...
        $scope.init = function ()
        {
          $("#volver").attr("href","{{ url('/admin/orden_examenes26?m=102') }}");
          if("{{$operation == 'update'}}"){

            $("#id_medico").val("{{$orden->id_medico}}").trigger("change");
            $scope.setRadio('id_tipo_orden',"{{$orden->id_tipo_orden}}");
            $("#id_tipo_orden").val("{{$orden->id_tipo_orden}}").val("{{$orden->id_tipo_orden}}");
          ...
          $scope.id_paciente = "{{($operation == 'update')?$orden->id_paciente :$paciente_ingresado}}";
                  ...
                  case 'add':
                    ...
                    }).then(function (response)
                    {
                      ...
                      if (response.data.response) {
                        ...
                        function(){
                          window.location = "{{ url('/admin/orden_examenes26') }}";
                        });
                      } else {
